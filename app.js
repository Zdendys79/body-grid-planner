...
// Displayed version derives from this script's own cache-buster query
// string (?v=N → "N/100" formatted to 2 decimals) — bumping ?v=N,
// already required by the Cache buster rule, is the only edit needed to
// update the H1 too. Must run at top level: document.currentScript is
// only set synchronously during this script's own initial execution.
const APP_VERSION = (() => {
  const m = document.currentScript && document.currentScript.src.match(/\?v=(\d+)/);
  return m ? (Number(m[1]) / 100).toFixed(2) : '?';
})();
...
// Reflects currentSaWorkers state on the merged SMART/STOP toggle button.
function updateAnnealButton() {
  const btn = document.getElementById('btn-anneal');
  if (!btn) return;
  const running = typeof currentSaWorkers !== 'undefined' && currentSaWorkers.length > 0;
  btn.classList.toggle('running', running);
  btn.textContent = running ? '■ STOP SMART' : '⚡³ SMART (Simulated Annealing)';
}
...
async function init() {
  ...
  try {
    // window.COMPONENTS_DATA is pre-loaded from components-data.js (generated
    // by build.js) — required for file:// usage where fetch() is blocked.
    const data = window.COMPONENTS_DATA
      || await fetch('components.json').then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); });
...
async function addComponent(componentId) {
  ...
  // findBestPlacement only wires up the NEW component — it doesn't revisit
  // any other already-placed component that was previously unreachable but
  // may now be one hop from power thanks to this addition (e.g. a battery
  // dropped next to a stranded Harvester). Re-scan the whole layout so those
  // get picked up too. tryAddWires only adds wires for still-unpowered
  // components — never moves or removes anything already placed.
  const rewired = tryAddWires(state.placements, state.grid);
  if (rewired) state.placements = rewired;
...
function expandBody() {
  ...
  stopOptimization(); // stop before mutating state
  ...
  let next = currentMax % 2 === 0 ? currentMax + 1 : currentMax + 2;
  // The game itself skips the 13×12 step entirely — once cols hit their
  // 12 cap, progression jumps straight from 11×11 to 15×12.
  if (next === 13) next = 15;
  grid.rows = Math.min(next, grid.maxRows);
  grid.cols = Math.min(next, grid.maxCols);

  const deltaRows = grid.rows - oldRows;
  // The game adds new rows at the TOP and new cols at the RIGHT, so all
  // existing components shift down by deltaRows. Wires are dropped and
  // recomputed because the S-bus row changed and old paths are now stale.
  if (deltaRows > 0) {
    state.placements = state.placements
      .filter(p => p.componentId !== 'wire')
      .map(p => ({ ...p, row: p.row + deltaRows }));
    const wired = tryAddWires(state.placements, grid);
    if (wired) state.placements = wired;
  }

  console.log(`[Expand] ${oldRows}×${oldCols} → ${grid.rows}×${grid.cols} (↓${deltaRows} rows)`);
  optResultsClear(); // saved layouts no longer fit the new grid
  saveState();
  renderAll();
...
async function optimizeAll() {
  ...
  // Debug stats snapshot (src/ui/debug-stats.js) — local-only, lets the user
  // export a before/after record when RE-OPTIMIZE doesn't seem to help.
  const statsBase = {
    gridRows: state.grid.rows, gridCols: state.grid.cols,
    componentCount: ids.length,
    scoreBefore: scoreLayout(savedPlacements, state.grid),
    wiresBefore: savedPlacements.filter(p => p.componentId === 'wire').length,
    workingBefore: computeWorkingSet(savedPlacements).size
  };
  ...
  if (skipped.length > 0) {
    ...
    reoptStatsRecord({ ...statsBase, outcome: 'rollback-skipped', skipped: uniqueNames });
  ...
  if (!isLayoutValid(state.placements, state.grid)) {
    reoptStatsRecord({ ...statsBase, outcome: 'rollback-invalid' });
    state.placements = savedPlacements;
    state.nextId = savedNextId;
    selectedPlacementIdx = null;
    saveState();
  ...
  const scoreAfter = scoreLayout(state.placements, state.grid);
  reoptStatsRecord({
    ...statsBase,
    scoreAfter,
    delta: scoreAfter - statsBase.scoreBefore,
    wiresAfter: state.placements.filter(p => p.componentId === 'wire').length,
    workingAfter: computeWorkingSet(state.placements).size,
    outcome: scoreAfter > statsBase.scoreBefore ? 'improved' : scoreAfter < statsBase.scoreBefore ? 'worse' : 'unchanged'
  });
...
function ensureComponentOrder(ids) {
  ...
  // they go BEFORE Repeaters/Spinners, not after: a Biocell often has only
  // one or two valid cells (right against its Generator's port), and an
  // unrelated Repeater/Spinner placed first can grab that exact cell,
  // starving the Biocell into the geometry-only fallback (which ignores
  // the hard constraint) or an outright skip. Biocells never compete with
  // Repeater/Spinner for cells — their search is confined to Bio Generator
  // neighbours — so moving them earlier costs nothing.
  ...
  const other    = ids.filter(id => !BIOCELL_IDS.has(id) && !repeaterSet.has(id) && !spinnerSet.has(id));
  ...
  return [...other, ...bioOnly, ...interleaved];
}
...
// Merged SMART/STOP button: starts a run when idle, stops it when running.
function toggleAnneal() {
  if (currentSaWorkers.length > 0) {
    stopAllWorkers();
  } else {
    startAnneal();
  }
...
function startAnneal() {
  ...
  stopOptimization();
  // Every fresh start clears prior results — a new run's leaves are scored
  // against the current layout/weights, so mixing in stale results from an
  // earlier session (different layout or weights) would be misleading.
  optResultsClear();
  autoFollowTop = true;
  renderOptResults();
  scheduleAnnealOpt();
...
function scheduleAnnealOpt() {
  ...
  const currentNonWire = state.placements.filter(p => p.componentId !== 'wire');
  ...
  function renderProgress() {
    ...
    optEl.textContent = `⚡³ ${N}× SA · iter ${fmtSci(totalIter)} · best cost ${fmt(avgBest)} · ${fmtElapsed(elapsedSec)} elapsed`;
  }
  ...
  for (let i = 0; i < N; i++) {
    ...
    const w = new Worker(window.SA_WORKER_BLOB_URL || 'sa-worker.js?v=147');
    ...
    w.onmessage = (e) => {
      ...
      switch (msg.type) {
        case 'ready': {
          // Temperature must scale with the score weights, or the Metropolis
          // acceptance exp(-delta/T) collapses to ~0 for every real move and
          // SA degenerates into pure hill-climbing (stuck at the first local
          // optimum, never escapes). Base it on the single largest weight so
          // a typical dominant-signal swing (e.g. losing one working Spinner)
          // still has a real chance of being accepted early in the run —
          // this keeps working automatically if weights get retuned again.
          const weights = getScoreWeights();
          const dominantWeight = Math.max(
            weights.workingSet, weights.amplifier, weights.batteryAmplifier,
            weights.energyAmpBioGen, weights.energyAmpEnergyCells, weights.energyAmpSpinner, weights.energyAmpPulser,
            weights.concentrator, weights.cluster
          );
          const baseTStart = dominantWeight * 0.6;
          const baseTEnd   = dominantWeight * 0.000002; // same tEnd/tStart ratio as the original 0.1/50000 calibration
          ...
          const opts = {
            tStart: baseTStart + i * (baseTStart * 0.1667),
            tEnd: baseTEnd,
    ...
    w.postMessage({ type: 'init', componentLib, scoreWeights: getScoreWeights() });
  }
}
