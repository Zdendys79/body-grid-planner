...
// string (e.g. "app.js?v=115" → "1.15") — bumping ?v=N, already required
// by the Cache buster rule, is the only edit needed to update the H1 too.
// Must run at top level: document.currentScript is only set synchronously
// during this script's own initial execution.
const APP_VERSION = (() => {
...
function scheduleAnnealOpt() {
  ...
  for (let i = 0; i < N; i++) {
    ...
    const w = new Worker(window.SA_WORKER_BLOB_URL || 'sa-worker.js?v=115');
