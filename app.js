...
function scheduleAnnealOpt() {
  ...
  for (let i = 0; i < N; i++) {
    ...
    const w = new Worker(window.SA_WORKER_BLOB_URL || 'sa-worker.js?v=141');
      ...
      switch (msg.type) {
        case 'ready': {
          ...
          const dominantWeight = Math.max(
            w.workingSet, w.amplifier, w.batteryAmplifier,
            w.energyAmpBioGen, w.energyAmpEnergyCells, w.energyAmpSpinner, w.energyAmpPulser,
            w.concentrator, w.cluster
          );
