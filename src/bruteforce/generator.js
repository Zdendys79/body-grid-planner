// src/bruteforce/generator.js — Canonical brute force generator.
// Shared between main thread (loaded via <script>) and the worker thread
// (loaded via importScripts). Reads componentLib as a global.
//
// Pruning optimisations applied at each node:
//   1. Unique rotations         — skip geometrically equivalent rotations
...
//   5. Repeater target          — prune Repeater that doesn't connect to Spinner/Pulser
//   6. Bus reachability         — BFS through free cells + direct bus + port-match
//   7. Branch slicing (Phase 2) — depth-0 branchStart/branchEnd for parallel workers
//
// Performance optimisations:
//   A. Flat Int32Array occupied grid (no Map allocation, no string keys)
//   B. Incremental push/pop with peripheral/energy-port tracking
//   C. Pre-cached rotation data per (id, deg): shape, cellOffsets, bounds,
//      ports, peri offsets, positions list
//   D. Integer port keys for all port lookups (no string hashing)
//   E. BFS uses generation counter, pre-allocated typed-array buffers
//   F. Incremental Spinner-Rep coverage counter — unmet count is O(1) read
//   G. Refcounted targetCoverKeyCount union of Spinner + Pulser cover keys
//
  ...
  function* search(idx, resume) {
    ...
    for (let ri = startRi; ri < rots.length; ri++) {
      ...
      for (let pi = piFrom; pi < len; pi += 2) {
        ...
        // Depth-0 branch-range slicing for parallel workers.
