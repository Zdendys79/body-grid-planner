...
- **`scoreLayout` signals** (biggest first, default weights): `workingSet × 2650000` → `computeAmplifierBonus` (Power Amplifier↔Harvester/Salvager port connections, 4000000 each) → `computeFreeBlockBonus` returns `{free, bus}` (powered free rectangles, super-linear; `.bus` is the subset touching the W/S bus directly, weighted separately from `.free` since v=128) → `quality × 15000` → `computeClusterBonus` (aesthetic same-type adjacency, 50000/pair) → `wires × −5000`. All seven weights are player-tunable in Settings — see `setScoreWeights`/`getScoreWeights` in `src/optimizer/score.js`.
