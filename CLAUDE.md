...
- **`scoreLayout` signals** (biggest first, default weights): `workingSet × 50000` → `computeFreeBlockBonus` (powered free rectangles, super-linear) → `computeAmplifierBonus` (Power Amplifier↔Harvester/Salvager port connections, 8000 each) → `wires × −5000` → `quality × 4` → `computeClusterBonus` (aesthetic same-type adjacency). All six weights are player-tunable in Settings — see `setScoreWeights`/`getScoreWeights` in `src/optimizer/score.js`.
