...
- **Layout validity** (`isLayoutValid`) requires: every energy-bearing component is powered, every Spinner is working (if any Repeater exists), every Repeater is port-adjacent to at least one Spinner or Pulser, every Biocell/Disposable Biocell is port-adjacent to a Bio Generator (any tier, including Bio Core), every Bio Core is port-adjacent to a Biocell/Disposable Biocell (the reverse requirement — unlike Bio Generator (I)/(II), a Bio Core does nothing without one), every Upgrader's pinned port(s) stay port-adjacent to their exact pinned instance (by `pinTag`/`pinnedTags`, set only via direct user placement/carry-drop — see `_repinUpgrader` in app.js) and its unpinned port(s) touch no other real component (`_upgraderPortAssignmentOk` in `src/optimizer/validate.js`) — an Upgrader with zero active pins is exempt from the "must be powered" requirement.
- **`scoreLayout` signals** (biggest first, default weights): `workingSet × 2650000` → `computeBatteryAmplifierBonus` (Battery Amplifier↔battery port connections, 1000000 × connected battery's cell count) → `computeEnergyAmplifierBonus` (Energy Amplifier↔Bio Generator/Energy Cells/Spinner/Pulser port connections, each target type its own weight — `energyAmpBioGen`/`energyAmpEnergyCells`/`energyAmpSpinner`/`energyAmpPulser`, 3000000 each by default, flat) → `computeAmplifierBonus` (Power Amplifier↔Harvester/Salvager port connections, 4000000 each) → `computeConcentratorBonus` (Concentrator↔Energy Cells connections, 3000000 each, counted per distinct connected block — a block touching via 2 of its own ports still counts once, same rule as the other 3 amplifier-family bonuses, fixed v=148 per GitHub issue #2) → `computeFreeBlockBonus` returns `{free, bus}` (powered free rectangles, super-linear, weighted ×0.2 by default since v=149 — was ×1 — to leave more headroom for the wire penalty below; `.bus` is the subset touching the W/S bus directly, weighted separately from `.free` since v=128) → `quality × 15000` → `computeClusterBonus` (aesthetic same-type adjacency, 50000/pair) → `wires × −100000` (raised from −5000 in v=149, per user preference for fewer wires over maximal open space). All thirteen weights are player-tunable in Settings — see `setScoreWeights`/`getScoreWeights` in `src/optimizer/score.js`.
