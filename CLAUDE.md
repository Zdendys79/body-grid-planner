...
- **Layout validity** (`isLayoutValid`) requires: every energy-bearing component is powered, every Spinner is working (if any Repeater exists), every Repeater is port-adjacent to at least one Spinner or Pulser, every Biocell/Disposable Biocell is port-adjacent to a Bio Generator (any tier).
