...
- **`scoreLayout` signals** (biggest first, default weights): `workingSet × 50000` → `computeFreeBlockBonus` returns `{free, bus}` (powered free rectangles, super-linear; `.bus` is the subset touching the W/S bus directly, weighted separately from `.free` since v=128) → `computeAmplifierBonus` (Power Amplifier↔Harvester/Salvager port connections, 8000 each) → `wires × −5000` → `quality × 4` → `computeClusterBonus` (aesthetic same-type adjacency). All seven weights are player-tunable in Settings — see `setScoreWeights`/`getScoreWeights` in `src/optimizer/score.js`.
