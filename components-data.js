// Generated by build.js — do not edit directly.
window.COMPONENTS_DATA = {
  "version": "1.0",
  "components": [
    {
      "id": "wire",
      "name": "Wire",
      "category": "infrastructure",
      "shape": [[0,0]],
      "energyPorts": [
        {"cell": [0,0], "side": "N"},
        {"cell": [0,0], "side": "E"},
        {"cell": [0,0], "side": "S"},
        {"cell": [0,0], "side": "W"}
      ],
      "peripheral": null,
      "color": "#FF7B2B",
      "bgColor": "#1e0a00",
      "icon": "✦",
      "description": "Ports N, E, S, W on (0,0)."
    },
    {
      "id": "battery_1x1",
      "name": "Battery (1x1)",
      "category": "power",
      "shape": [[0,0]],
      "energyPorts": [
        {"cell": [0,0], "side": "W"},
        {"cell": [0,0], "side": "E"}
      ],
      "peripheral": null,
      "color": "#EF5350",
      "bgColor": "#4a0f0f",
      "icon": "▓",
      "description": "Ports W, E on (0,0)."
    },
    {
      "id": "battery_1x2",
      "name": "Battery (1x2)",
      "category": "power",
      "shape": [[0,0],[1,0]],
      "energyPorts": [
        {"cell": [1,0], "side": "S"}
      ],
      "peripheral": null,
      "color": "#EF5350",
      "bgColor": "#4a0f0f",
      "icon": "▓",
      "description": "S on (1,0)."
    },
    {
      "id": "battery_1x3",
      "name": "Battery (1x3)",
      "category": "power",
      "shape": [[0,0],[0,1],[0,2]],
      "energyPorts": [
        {"cell": [0,0], "side": "W"},
        {"cell": [0,2], "side": "E"}
      ],
      "peripheral": null,
      "color": "#EF5350",
      "bgColor": "#4a0f0f",
      "icon": "▓",
      "description": "W on (0,0). E on (0,2)."
    },
    {
      "id": "battery_2x2",
      "name": "Battery (2x2)",
      "category": "power",
      "shape": [[0,0],[0,1],[1,0],[1,1]],
      "energyPorts": [
        {"cell": [0,0], "side": "N"}
      ],
      "peripheral": null,
      "color": "#EF5350",
      "bgColor": "#4a0f0f",
      "icon": "▓",
      "description": "N on (0,0)."
    },
    {
      "id": "battery_3x2",
      "name": "Battery (3x2)",
      "category": "power",
      "shape": [[0,0],[0,1],[0,2],[1,0],[1,1],[1,2]],
      "energyPorts": [
        {"cell": [0,2], "side": "E"},
        {"cell": [1,2], "side": "E"}
      ],
      "peripheral": null,
      "color": "#EF5350",
      "bgColor": "#4a0f0f",
      "icon": "▓",
      "description": "Battery (3x2). E on (0,2). E on (1,2)."
    },
    {
      "id": "sensor",
      "name": "Sensor",
      "category": "detection",
      "shape": [[0,0],[0,1],[1,0],[2,0],[3,0],[4,0]],
      "energyPorts": [
        {"cell": [4,0], "side": "W"},
        {"cell": [4,0], "side": "S"},
        {"cell": [4,0], "side": "E"}
      ],
      "peripheral": null,
      "color": "#26C6DA",
      "bgColor": "#00303a",
      "icon": "◎",
      "description": "Ports W, S, E on (4,0)."
    },
    {
      "id": "bio_generator",
      "name": "Bio Generator",
      "category": "power",
      "shape": [[0,1],[0,2],[1,0],[1,1],[1,2]],
      "energyPorts": [
        {"cell": [1,0], "side": "N"},
        {"cell": [1,0], "side": "W"}
      ],
      "peripheral": null,
      "color": "#66BB6A",
      "bgColor": "#062010",
      "icon": "☘",
      "description": "Bio Generator, 2x3 (no integrated Biocell — attach one separately). N+W on (1,0)."
    },
    {
      "id": "bio_generator_ii",
      "name": "Bio Generator (II)",
      "category": "power",
      "shape": [[0,0],[0,1],[1,0],[1,1],[2,0],[2,1],[2,2]],
      "energyPorts": [
        {"cell": [2,0], "side": "W"},
        {"cell": [2,2], "side": "N"},
        {"cell": [2,2], "side": "E"}
      ],
      "peripheral": null,
      "color": "#66BB6A",
      "bgColor": "#062010",
      "icon": "☘",
      "description": "Bio Generator tier II, redrawn to match the in-game shape (no integrated Biocell — attach one separately). W on (2,0). N+E on (2,2)."
    },
    {
      "id": "energy_cells",
      "name": "Energy Cells",
      "category": "power",
      "shape": [[0,0],[0,1],[1,0],[1,1]],
      "energyPorts": [
        {"cell": [0,0], "side": "W"},
        {"cell": [0,1], "side": "E"},
        {"cell": [1,0], "side": "W"},
        {"cell": [1,1], "side": "E"}
      ],
      "peripheral": null,
      "color": "#FFB74D",
      "bgColor": "#2a1500",
      "icon": "⬡",
      "description": "W on (0,0). E on (0,1). W on (1,0). E on (1,1)."
    },
    {
      "id": "power_amplifier",
      "name": "Power Amplifier",
      "category": "power",
      "shape": [[0,0],[0,1],[1,0],[1,1]],
      "energyPorts": [
        {"cell": [1,0], "side": "W"},
        {"cell": [1,1], "side": "E"},
        {"cell": [1,1], "side": "S"}
      ],
      "peripheral": null,
      "color": "#FFD54F",
      "bgColor": "#332400",
      "icon": "⚡",
      "optimizerRules": {
        "connectsTo": ["harvester", "salvager"],
        "note": "Boosts the output of any port-connected Harvester or Salvager. Optional — not required for layout validity, but scoreLayout rewards each connection (Settings → Layout scoring weights → Amplifier bonus)."
      },
      "description": "Power Amplifier, 2x2. W on (1,0). E+S on (1,1). Boosts output of a port-connected Harvester or Salvager."
    },
    {
      "id": "battery_amplifier",
      "name": "Battery Amplifier",
      "category": "power",
      "shape": [[0,0],[0,1],[1,1]],
      "energyPorts": [
        {"cell": [1,1], "side": "W"},
        {"cell": [1,1], "side": "E"}
      ],
      "peripheral": null,
      "color": "#FFD54F",
      "bgColor": "#332400",
      "icon": "⚡",
      "optimizerRules": {
        "connectsTo": ["battery_1x1", "battery_1x2", "battery_1x3", "battery_2x2", "battery_3x2"],
        "note": "Boosts the capacity of a port-connected battery, scaled by that battery's cell count — bigger batteries gain proportionally more. Optional — not required for layout validity, but scoreLayout rewards each connection (Settings → Layout scoring weights → Battery Amplifier bonus)."
      },
      "description": "Battery Amplifier, 3 cells. W+E on (1,1). Boosts capacity of a port-connected battery, scaled by its size."
    },
    {
      "id": "energy_amplifier",
      "name": "Energy Amplifier",
      "category": "power",
      "shape": [[0,0],[1,0],[2,0]],
      "energyPorts": [
        {"cell": [2,0], "side": "W"},
        {"cell": [2,0], "side": "E"}
      ],
      "peripheral": null,
      "color": "#FFD54F",
      "bgColor": "#332400",
      "icon": "⚡",
      "optimizerRules": {
        "connectsTo": ["bio_generator", "bio_generator_ii", "energy_cells", "spinner", "pulser"],
        "note": "Boosts the efficiency of a port-connected energy producer (Bio Generator, Energy Cells, Spinner, Pulser). Optional — not required for layout validity, but scoreLayout rewards each connection with its own per-target-type weight (Settings → Layout scoring weights → Energy Amp: Bio Generator / Energy Cells / Spinner / Pulser)."
      },
      "description": "Energy Amplifier, 3 cells (vertical). W+E on (2,0). Boosts efficiency of a port-connected energy producer."
    },
    {
      "id": "concentrator",
      "name": "Concentrator",
      "category": "power",
      "shape": [[0,0],[0,1],[1,0],[1,1]],
      "energyPorts": [
        {"cell": [0,0], "side": "N"},
        {"cell": [0,0], "side": "W"},
        {"cell": [0,1], "side": "N"},
        {"cell": [0,1], "side": "E"},
        {"cell": [1,0], "side": "S"},
        {"cell": [1,0], "side": "W"},
        {"cell": [1,1], "side": "S"},
        {"cell": [1,1], "side": "E"}
      ],
      "peripheral": null,
      "color": "#FFD54F",
      "bgColor": "#332400",
      "icon": "✳",
      "optimizerRules": {
        "connectsTo": ["energy_cells"],
        "note": "Boosts the energy generated by a port-connected Energy Cells block. Optional — not required for layout validity, but scoreLayout rewards each connection (Settings → Layout scoring weights → Concentrator bonus)."
      },
      "description": "Concentrator, 2x2, ports on all 8 outward sides (N+W, N+E, S+W, S+E on its four corners). Boosts energy generated by a port-connected Energy Cells block."
    },
    {
      "id": "pulser",
      "name": "Pulser",
      "category": "power",
      "shape": [[0,0],[0,1],[1,0],[1,1]],
      "energyPorts": [
        {"cell": [1,0], "side": "W"},
        {"cell": [1,1], "side": "E"}
      ],
      "peripheral": null,
      "color": "#CE93D8",
      "bgColor": "#1a0a22",
      "icon": "◈",
      "description": "W on (1,0). E on (1,1)."
    },
    {
      "id": "spinner",
      "name": "Spinner",
      "category": "processing",
      "shape": [[0,0],[0,1],[1,0],[1,1],[2,0],[2,1]],
      "energyPorts": [
        {"cell": [2,0], "side": "W"},
        {"cell": [2,1], "side": "E"}
      ],
      "peripheral": null,
      "color": "#4FC3F7",
      "bgColor": "#001830",
      "icon": "↺",
      "optimizerRules": {
        "requires": "2s_pulse",
        "sources": [
          {"component": "repeater_2s", "count": 1, "sides": "any"},
          {"component": "repeater_4s", "count": 2, "sides": "both"}
        ]
      },
      "description": "W on (2,0). E on (2,1). Requires: 1× Repeater (2s) on any side, OR 1× Repeater (4s) on each side. One Repeater may serve up to 2 Spinners/Pulsers."
    },
    {
      "id": "repeater_2s",
      "name": "Repeater (2s)",
      "category": "timing",
      "shape": [[0,0],[1,0]],
      "energyPorts": [
        {"cell": [1,0], "side": "W"},
        {"cell": [1,0], "side": "E"}
      ],
      "peripheral": null,
      "color": "#80CBC4",
      "bgColor": "#00251a",
      "icon": "⟳",
      "intervalMs": 2000,
      "affects": ["pulser","spinner"],
      "optimizerRules": {
        "connectsTo": ["pulser","spinner"],
        "bothSides": true,
        "note": "Spinner needs 2s pulses: one 2s repeater on either side is sufficient."
      },
      "description": "W, E on (1,0). Interval 2s. Must be placed directly adjacent (W or E side) to Spinner or Pulser. One 2s Repeater on any side is sufficient for one or two Spinners."
    },
    {
      "id": "repeater_4s",
      "name": "Repeater (4s)",
      "category": "timing",
      "shape": [[0,0],[1,0]],
      "energyPorts": [
        {"cell": [1,0], "side": "W"},
        {"cell": [1,0], "side": "E"}
      ],
      "peripheral": null,
      "color": "#80CBC4",
      "bgColor": "#001a12",
      "icon": "⟳",
      "intervalMs": 4000,
      "affects": ["pulser","spinner"],
      "optimizerRules": {
        "connectsTo": ["pulser","spinner"],
        "bothSides": true,
        "note": "Spinner needs 2s pulses: requires one 4s repeater on EACH side to achieve 2s combined."
      },
      "description": "W, E on (1,0). Interval 4s. Must be placed on BOTH sides of a Spinner/Pulser (one on each side) to be effective. Can serve as the shared Repeater between two Spinners."
    },
    {
      "id": "grabber",
      "name": "Grabber",
      "category": "processing",
      "shape": [[0,0],[0,1],[1,0],[1,1],[2,0],[2,1],[3,0],[3,1]],
      "energyPorts": [
        {"cell": [3,0], "side": "W"},
        {"cell": [3,1], "side": "E"}
      ],
      "peripheral": null,
      "color": "#FF8A65",
      "bgColor": "#1a0800",
      "icon": "⊞",
      "description": "W on (3,0). E on (3,1)."
    },
    {
      "id": "collector",
      "name": "Collector (I)",
      "category": "processing",
      "shape": [[0,0],[0,1],[0,2],[0,3],[0,4]],
      "energyPorts": [
        {"cell": [0,0], "side": "W"}
      ],
      "peripheral": null,
      "color": "#A5D6A7",
      "bgColor": "#0a1a0a",
      "icon": "⊟",
      "description": "Collector tier I, 1x5. W on (0,0)."
    },
    {
      "id": "collector_ii",
      "name": "Collector (II)",
      "category": "processing",
      "shape": [[0,0],[1,0],[2,0]],
      "energyPorts": [
        {"cell": [1,0], "side": "W"},
        {"cell": [1,0], "side": "E"}
      ],
      "peripheral": null,
      "color": "#A5D6A7",
      "bgColor": "#0a1a0a",
      "icon": "⊟",
      "description": "Collector tier II, 1x3 (vertical). W+E on (1,0)."
    },
    {
      "id": "decomposer",
      "name": "Decomposer (I)",
      "category": "processing",
      "shape": [[0,1],[0,2],[0,3],[1,0],[1,1],[1,2]],
      "energyPorts": [
        {"cell": [1,0], "side": "W"},
        {"cell": [1,2], "side": "E"}
      ],
      "peripheral": null,
      "color": "#CE93D8",
      "bgColor": "#1a0a22",
      "icon": "⊠",
      "description": "Decomposer tier I. W on (1,0). E on (1,2)."
    },
    {
      "id": "decomposer_ii",
      "name": "Decomposer (II)",
      "category": "processing",
      "shape": [[0,0],[0,1],[1,0],[1,1],[1,2],[1,3],[2,0],[2,1],[2,2],[2,3]],
      "energyPorts": [
        {"cell": [0,0], "side": "W"},
        {"cell": [0,1], "side": "E"}
      ],
      "peripheral": null,
      "color": "#CE93D8",
      "bgColor": "#1a0a22",
      "icon": "⊠",
      "description": "Decomposer tier II. W on (0,0). E on (0,1)."
    },
    {
      "id": "harvester",
      "name": "Harvester",
      "category": "processing",
      "shape": [[0,1],[0,2],[1,0],[1,1],[1,2],[1,3]],
      "energyPorts": [
        {"cell": [0,1], "side": "W"},
        {"cell": [0,2], "side": "E"}
      ],
      "peripheral": null,
      "color": "#A5D6A7",
      "bgColor": "#0a1500",
      "icon": "⊕",
      "description": "W on (0,1). E on (0,2)."
    },
    {
      "id": "salvager",
      "name": "Salvager",
      "category": "processing",
      "shape": [[0,0],[0,1],[0,2],[1,1],[1,2]],
      "energyPorts": [
        {"cell": [0,0], "side": "W"}
      ],
      "peripheral": null,
      "color": "#FFB74D",
      "bgColor": "#1a0f00",
      "icon": "⊗",
      "description": "W on (0,0)."
    },
    {
      "id": "metal_scavenger",
      "name": "Metal Scavenger",
      "category": "processing",
      "shape": [[0,0],[0,1],[0,2],[1,0],[1,1],[1,2]],
      "energyPorts": [
        {"cell": [1,0], "side": "W"}
      ],
      "peripheral": null,
      "color": "#B0BEC5",
      "bgColor": "#0e1419",
      "icon": "⊡",
      "description": "Metal harvester 2x3. W on (1,0)."
    },
    {
      "id": "resource_scanner",
      "name": "Resource Scanner",
      "category": "processing",
      "shape": [[0,2],[1,0],[1,1],[1,2],[1,3],[1,4]],
      "energyPorts": [
        {"cell": [0,2], "side": "N"}
      ],
      "peripheral": null,
      "color": "#80DEEA",
      "bgColor": "#00272b",
      "icon": "⌖",
      "description": "Resource Scanner, 6 cells (T-shape). N on (0,2). Boosts yield of found metals."
    },
    {
      "id": "furnace",
      "name": "Furnace (I)",
      "category": "processing",
      "shape": [[0,0],[0,1],[0,2],[1,0],[1,1],[1,2]],
      "energyPorts": [
        {"cell": [1,0], "side": "W"},
        {"cell": [1,2], "side": "E"}
      ],
      "peripheral": null,
      "color": "#FF7043",
      "bgColor": "#1a0500",
      "icon": "⊙",
      "description": "Furnace tier I, 2x3. W on (1,0). E on (1,2)."
    },
    {
      "id": "furnace_ii",
      "name": "Furnace (II)",
      "category": "processing",
      "shape": [[0,0],[0,1],[0,2],[1,0],[1,1],[1,2],[2,0],[2,1],[2,2],[3,0],[3,1],[3,2]],
      "energyPorts": [
        {"cell": [3,0], "side": "W"},
        {"cell": [3,0], "side": "S"},
        {"cell": [3,2], "side": "E"},
        {"cell": [3,2], "side": "S"}
      ],
      "peripheral": null,
      "color": "#FF7043",
      "bgColor": "#1a0500",
      "icon": "⊙",
      "description": "Furnace tier II, 4x3. W+S on (3,0). E+S on (3,2)."
    },
    {
      "id": "fuser_i",
      "name": "Fuser (I)",
      "category": "processing",
      "shape": [[0,0],[0,1],[0,2],[0,3],[1,0],[1,1],[1,2],[1,3],[2,0],[2,1],[2,2],[2,3],[3,0],[3,1],[3,2],[3,3]],
      "energyPorts": [
        {"cell": [0,0], "side": "W"},
        {"cell": [3,0], "side": "W"},
        {"cell": [3,3], "side": "E"}
      ],
      "peripheral": null,
      "color": "#4DD0E1",
      "bgColor": "#002025",
      "icon": "⊜",
      "description": "Fuser tier I, 4x4. W on (0,0) and (3,0). E on (3,3)."
    },
    {
      "id": "disposable_biocell",
      "name": "Disposable Biocell",
      "category": "bio",
      "shape": [[0,0]],
      "energyPorts": [
        {"cell": [0,0], "side": "S"}
      ],
      "peripheral": null,
      "color": "#81C784",
      "bgColor": "#062010",
      "icon": "◌",
      "optimizerRules": {
        "connectsTo": ["bio_generator", "bio_generator_ii"],
        "note": "Normal electrical port, but only functions when plugged directly into a Bio Generator (any tier). Hard requirement — a layout with a Disposable Biocell not port-adjacent to a Bio Generator is invalid."
      },
      "description": "Disposable Biocell, 1 cell. S on (0,0). Must be port-adjacent to a Bio Generator."
    },
    {
      "id": "biocell",
      "name": "Biocell",
      "category": "bio",
      "shape": [[0,0],[1,0]],
      "energyPorts": [
        {"cell": [1,0], "side": "S"}
      ],
      "peripheral": null,
      "color": "#81C784",
      "bgColor": "#062010",
      "icon": "◉",
      "optimizerRules": {
        "connectsTo": ["bio_generator", "bio_generator_ii"],
        "note": "Normal electrical port, but only functions when plugged directly into a Bio Generator (any tier). Hard requirement — a layout with a Biocell not port-adjacent to a Bio Generator is invalid. Same shape as Refilling Biocell (not modeled separately — the optimizer doesn't distinguish consumable vs rechargeable)."
      },
      "description": "Biocell, 2 cells (vertical). S on (1,0). Must be port-adjacent to a Bio Generator."
    }
  ]
}
;
